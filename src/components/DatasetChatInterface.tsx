...
const DatasetChatInterface: React.FC<DatasetChatInterfaceProps> = ({ datasetId, datasetName }) => {
  ...
  const getChartTypeIcon = (chartType?: string) => {
    switch(chartType) {
      case 'bar': return <BarChart className="h-4 w-4 text-purple-400" />;
      case 'line': return <LineChart className="h-4 w-4 text-blue-400" />;
      case 'pie': return <PieChart className="h-4 w-4 text-green-400" />;
      default: return <BarChart className="h-4 w-4 text-purple-400" />;
    }
  };
              ...
              {messages.map((message) => (
                ...
                >
                  <div 
                    className={`max-w-[85%] ${
                      message.sender === 'user' 
                        ? 'bg-blue-500/70 text-white rounded-t-xl rounded-bl-xl' 
                        : 'bg-purple-800/50 backdrop-blur-sm text-white rounded-t-xl rounded-br-xl'
                    } p-3 shadow-md`}
                  >
                    {message.sender === 'ai' && (
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button 
                                  ...
                                  size="icon" 
                                  className="h-7 w-7"
                                  onClick={() => message.result && downloadVisualization(message.result)}
                                >
