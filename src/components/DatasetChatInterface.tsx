...
const DatasetChatInterface: React.FC<DatasetChatInterfaceProps> = ({
  ...
  return <div className="flex flex-col h-[70vh] max-h-[70vh]">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-medium">Chat with Your Data</h2>
        <div className="flex space-x-2">
          <Button variant={activeModel === 'openai' ? 'default' : 'outline'} size="sm" onClick={() => setActiveModel('openai')} className="flex items-center gap-1">
          ...
          <Button variant={activeModel === 'anthropic' ? 'default' : 'outline'} size="sm" onClick={() => setActiveModel('anthropic')} className="flex items-center gap-1 bg-amber-600 hover:bg-amber-500">
            <Avatar className="h-5 w-5 mr-1">
          ...
          <CardContent className="pt-6 pb-2">
            <div className="flex flex-col space-y-4">
              {messages.map(message => <div key={message.id} className={`flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] ${message.sender === 'user' ? 'bg-blue-500/70 text-white rounded-t-xl rounded-bl-xl' : 'bg-purple-800/50 backdrop-blur-sm text-white rounded-t-xl rounded-br-xl'} p-3 shadow-md`}>
                    {message.sender === 'ai' && <div className="flex items-center mb-2">
                        <Avatar className="h-6 w-6 mr-2">
                          <AvatarImage src={message.model === 'anthropic' ? "https://upload.wikimedia.org/wikipedia/commons/2/28/Anthropic_Logo.png" : "https://upload.wikimedia.org/wikipedia/commons/0/04/ChatGPT_logo.svg"} />
                    ...
                    {message.result && <div className="mt-4">
      ...
      <div className="flex flex-wrap gap-2 mt-4">
        <Badge variant="outline" className="bg-purple-500/20 cursor-pointer hover:bg-purple-500/30" onClick={() => setInputText("Show me a breakdown of the data by region")}>
          Breakdown by region
        </Badge>
        <Badge variant="outline" className="bg-purple-500/20 cursor-pointer hover:bg-purple-500/30" onClick={() => setInputText("What's the trend over time?")}>
          Trend over time
        </Badge>
        <Badge variant="outline" className="bg-purple-500/20 cursor-pointer hover:bg-purple-500/30" onClick={() => setInputText("Compare top 5 categories")}>
          Top 5 comparison
        </Badge>
        <Badge variant="outline" className="bg-purple-500/20 cursor-pointer hover:bg-purple-500/30" onClick={() => setInputText("Show outliers in the dataset")}>
          Find outliers
        </Badge>
      </div>
    </div>;
