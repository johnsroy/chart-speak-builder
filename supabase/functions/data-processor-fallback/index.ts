  ...
  try {
    ...
    if (action === 'preview') {
      ...
      try {
        ...
        // If there's no schema, create a minimal sample
        if (sampleData.length === 0) {
          ...
          return new Response(
            JSON.stringify({
              ...
              success: true,
              message: "Fallback data generated - original file could not be accessed"
            }),
            { headers: { ...corsHeaders, "Content-Type": "application/json" } }
          );
        }
        ...
        return new Response(
          JSON.stringify({
            ...
            success: true,
            message: "Sample data generated - original file could not be accessed"
          }),
          { headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      } catch (error) {
